# shellcheck disable=SC2034,SC2276,SC2283,SC1072,SC1073
...
# FT245 timing profile: "pico" for the RP2040 bridge, "ft2232h" for a
# real FT2232H async FIFO.  The default preserves the Pico-compatible build.
FT245_TIMING?=pico

.PHONY: all build build-pico build-ft2232h lint prog flash clean tool tool-d2xx ftdi-setup help
...
# Force-rebuild convenience targets for the two timing variants.  Both emit
# impl/pnr/spi_flash.fs, so the later target overwrites the earlier bitstream.
build-pico:
	$(MAKE) -B $(BITSTREAM) FT245_TIMING=pico

build-ft2232h:
	$(MAKE) -B $(BITSTREAM) FT245_TIMING=ft2232h

$(BITSTREAM): $(VERILOG_FILES) $(CST_FILE) build.tcl
	FT245_TIMING=$(FT245_TIMING) gw_sh build.tcl

# Lint with yosys (quick syntax/logic check)
lint:
	mkdir -p impl/generated
	if [ "$(FT245_TIMING)" = "ft2232h" ]; then \
		printf '%s\n%s\n' '// Auto-generated by Makefile lint; do not edit.' '`define FT245_TIMING_FT2232H 1' > impl/generated/ft245_timing.vh; \
	else \
		printf '%s\n%s\n' '// Auto-generated by Makefile lint; do not edit.' '`define FT245_TIMING_PICO 1' > impl/generated/ft245_timing.vh; \
	fi
...
help:
	...
	@echo ""
	@echo "  make build   - Synthesize + PnR (default: FT245_TIMING=pico)"
	@echo "  make build-pico - Force-build Pico/RP2040 FT245 timing variant"
	@echo "  make build-ft2232h - Force-build real FT2232H timing variant"
