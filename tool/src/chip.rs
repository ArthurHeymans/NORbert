...
use rflasher_core::chip::{ChipDatabase as RflasherChipDatabase, Features};
...
fn chip_from_rflasher(chip: &rflasher_core::chip::FlashChip) -> FlashChip {
    let erase_ops = chip
        .erase_blocks
        .iter()
        .filter_map(|block| {
            block.regions().first().map(|region| EraseOp {
                opcode: block.opcode,
    ...
    FlashChip {
        ...
        jedec_manufacturer: chip.jedec_manufacturer,
        jedec_device: chip.jedec_device,
        total_size: chip.total_size,
        page_size: chip.page_size,
        supports_4byte: chip.features.contains(Features::FOUR_BYTE_ADDR)
            || chip.features.contains(Features::FOUR_BYTE_ENTER),
        supports_dual: chip.features.contains(Features::DUAL_IO),
        supports_quad: chip.features.contains(Features::QUAD_IO),
        supports_fast_read: chip.features.contains(Features::FAST_READ),
        aai_word: chip.features.contains(Features::AAI_WORD),
        write_byte: chip.features.contains(Features::WRITE_BYTE),
        supports_sfdp: chip.features.contains(Features::SFDP),
        erase_ops,
    }
}

/// Load the static chip database generated by rflasher from its RON files.
pub fn load_embedded_chip_db() -> Vec<FlashChip> {
    RflasherChipDatabase::new()
        .chips()
        .iter()
        .map(chip_from_rflasher)
        .collect()
}
...
/// Convert a parsed rflasher vendor definition into NORbert chip entries.
pub fn chips_from_vendor_def(vendor_def: &VendorDef) -> Vec<FlashChip> {
    vendor_def
        .chips
        .iter()
        .map(|chip_def| {
            let erase_ops = chip_def
                .erase_blocks
                .iter()
                .filter_map(|eb| {
                    ...
                    eb.regions.first().map(|region| EraseOp {
                        opcode: eb.opcode,
...
/// Parse one rflasher-compatible vendor RON file.
pub fn parse_vendor_ron(content: &str, source: &str) -> Result<Vec<FlashChip>> {
    let vendor_def: VendorDef =
        ron::from_str(content).with_context(|| format!("Failed to parse {}", source))?;
    Ok(chips_from_vendor_def(&vendor_def))
}

/// Parse multiple rflasher-compatible vendor RON strings.
pub fn load_chip_db_from_ron_strings<'a>(
    items: impl IntoIterator<Item = (&'a str, &'a str)>,
) -> Result<Vec<FlashChip>> {
    let mut chips = Vec::new();
    for (source, content) in items {
        chips.extend(parse_vendor_ron(content, source)?);
    }
    Ok(chips)
}
...
pub fn load_chip_db(dir: &Path) -> Result<Vec<FlashChip>> {
    ...
    for entry in entries {
        ...
        chips.extend(parse_vendor_ron(&content, &path.display().to_string())?);
    }
