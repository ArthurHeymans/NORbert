...
// This implementation is derived from openFPGALoader's Gowin and FTDI JTAG
// paths. Copyright (C) 2019 Gwenhael Goavec-Merou and openFPGALoader contributors.
...
impl GowinProgrammer {
    ...
    async fn erase_sram(&mut self) -> Result<(), String> {
        ...
        for attempt in 1..=MAX_ERASE_ATTEMPTS {
            ...
            last_status = self.read_status().await?;
            if attempt >= 2 && (last_status & STATUS_DONE_FINAL) == 0 {
                return Ok(());
            }
        }
        Err(format!(
            "SRAM erase did not clear DONE after {MAX_ERASE_ATTEMPTS} attempts; status=0x{last_status:08x}"
        ))
    }
    ...
    async fn read_sfdp(&mut self, address: u32, len: usize) -> Result<Vec<u8>, String> {
        if u64::from(address) + len as u64 > 0x0100_0000 {
            return Err("SFDP table exceeds the 24-bit address range".to_string());
        }
        let mut payload = addr24(address).to_vec();
        payload.push(0); // READ SFDP requires one dummy byte after the address.
    ...
    async fn spi_flash_capacity(&mut self) -> Result<u64, String> {
        ...
        if &header[..4] != b"SFDP" || header[5] != 1 {
            return Err(
                "flash has no supported SFDP header; refusing to guess its capacity".to_string(),
            );
        }
...
struct WebUsbIdentity {
    product: String,
    serial: Option<String>,
}

impl WebUsbIdentity {
    fn linux_unbind_hint(&self) -> String {
        let Some(serial) = self.serial.as_deref() else {
            return "The selected device has no USB serial number, so the browser cannot generate a safe per-device Linux unbind command".to_string();
        };
        let serial = shell_quote(serial);
        format!(
            "To unbind only this selected device's interface A on Linux, run: SERIAL={serial}; for dev in /sys/bus/usb/devices/*; do [ -r \"$dev/serial\" ] && [ \"$(cat \"$dev/serial\")\" = \"$SERIAL\" ] && [ -e \"$dev:1.0/driver/unbind\" ] && echo \"${{dev##*/}}:1.0\" | sudo tee \"$dev:1.0/driver/unbind\"; done"
        )
    }
}
...
async fn request_ft2232h() -> Result<(nusb::Device, WebUsbIdentity), String> {
    ...
    let selected = JsFuture::from(window.navigator().usb().request_device(&options))
        .await
        .map_err(|error| format!("WebUSB FTDI request failed: {error:?}"))?;
    let usb_device: UsbDevice = selected
        .dyn_into()
        .map_err(|_| "browser returned an invalid USB device".to_string())?;
    let identity = WebUsbIdentity {
        product: usb_device
            .product_name()
            .unwrap_or_else(|| "FT2232H".to_string()),
        serial: usb_device.serial_number(),
    };
    let device = nusb::Device::from_js(usb_device)
        .await
        .map_err(|error| format!("failed to open WebUSB device: {error}"))?;
    Ok((device, identity))
}
...
impl FtdiJtag {
    async fn open_tang_primer_25k() -> Result<Self, String> {
        let (info, identity) = request_ft2232h().await?;
        let descriptor = info.device_descriptor();
        if descriptor.vendor_id() != FTDI_VID || descriptor.product_id() != FT2232H_PID {
            return Err(format!(
                "selected USB device is {:04x}:{:04x}; choose the Tang Primer 25K FT2232H ({FTDI_VID:04x}:{FT2232H_PID:04x})",
        ...
        let mut dev = FtdiDevice::open_wasm(info, Interface::A)
            .await
            .map_err(|e| {
                let serial = identity.serial.as_deref().unwrap_or("unavailable");
                format!(
                    "failed to open FT2232H interface A on selected device '{}' (serial {}): {e}. Disconnect active USB/JTAG tools. {}. On Windows, bind only this device's interface A to WinUSB",
                    identity.product,
                    serial,
                    identity.linux_unbind_hint(),
                )
            })?;
        ...
        let ctx = MpsseContext::init(&mut dev, 2_500_000)
            .await
    ...
    async fn sync_mpsse(&mut self) -> Result<(), String> {
        const SYNC_COMMAND: u8 = 0xaa;

        self.dev
            .flush_rx()
            .await
            .map_err(|e| format!("failed to flush FTDI MPSSE input: {e}"))?;
        self.dev
            .write_all(&[SYNC_COMMAND, mpsse::SEND_IMMEDIATE])
            .await
            .map_err(|e| format!("failed to write FTDI MPSSE sync: {e}"))?;
        let mut response = Vec::with_capacity(64);
        for _ in 0..64 {
            response.push(self.read_exact(1).await?[0]);
            if response.ends_with(&[0xfa, SYNC_COMMAND]) {
                return Ok(());
            }
        }
        Err(format!(
            "FTDI MPSSE sync did not return [fa, aa]; received {:02x?}",
            response
        ))
    }
    ...
    async fn read_exact(&mut self, len: usize) -> Result<Vec<u8>, String> {
        const MAX_STATUS_ONLY_READS: usize = 64;

        let mut buf = vec![0u8; len];
        let mut offset = 0usize;
        let mut status_only_reads = 0usize;
        while offset < len {
            let n = self
                .dev
            ...
            if n == 0 {
                status_only_reads += 1;
                if status_only_reads >= MAX_STATUS_ONLY_READS {
                    return Err("JTAG read timed out waiting for MPSSE data".to_string());
                }
                continue;
            }
            status_only_reads = 0;
